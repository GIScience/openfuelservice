...
@pytest.mark.asyncio
async def test_wikipedia_importer(db: Session) -> None:
    test_car_category = {
        "car_categories": {
            "a": {
                "category_names": [
    ...
    # Test wikicategories
    assert len(wiki_categories_in_db) == len(wikipedia_reader.categories)
    wiki_cars_via_reference: List = []
    compare_counter: int = 0
    for category in wikipedia_reader.categories:
    ...
    assert len(wiki_cars_in_db) == len(wikipedia_reader.wiki_cars)
    assert len(wiki_cars_in_db) == len(wiki_cars_via_reference)
    compare_counter = 0
    car: WikiCar
    for car in wikipedia_reader.wiki_cars:
        car_in_db: WikiCar
    ...
    for wiki_car_object in wikipedia_reader.wiki_cars:
        try:
            db.delete(wiki_car_object)
        except Exception:
            pass
    db.commit()
    for wiki_category_object in wikipedia_reader.categories:
        try:
            db.delete(wiki_category_object)
        except Exception:
            pass
    db.commit()
